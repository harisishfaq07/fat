...
class UsersController < ApplicationController
  ...
  def index
    ...
    if params[:suspend].present?
      # @users = @q.result.where.not(suspended_at: nil)
      @users = User.all.where.not(suspended_at: nil)
    else
      # @users = @q.result.where(admin: params[:admin])
      @users = User.all.where(admin: params[:admin])
    end
  ...
  def suspend_user
    ...
    if @user.update(suspended_at: Time.now)
      flash[:alert] = "User Update Successfully"
